...
;if (!window.af || typeof(af) !== "function") {
    ...
    var af = (function(window) {
        ...
        */
        $.jsonP = function(options) {
            ...
            var callbackName = 'jsonp_callback' + (+new Date()) + '_' + (_jsonPID++);
            var abortTimeout = "",
                context, callback;
            ...
            // modified by yuanlihao/65936 2013/12/9
        ...
        */
        ...
        // modified by yuanlihao/065936 2014/02/25
        /**
        * get script.
    ...
    // modified by yuanlihao/65936 2013/11/28
    //'$' in window || (window.$ = af);
...
/*
    add some prototype methods for app framework.
 */
(function($) {
    
    /**
     * 获取指定元素的computedStyle属性
    ...
    $.fn.outerWidth = function(needMargin) {
        return needMargin === true ? 
                $(this).width() + curCSS(this[0], 'marginLeft') + curCSS(this[0], 'marginRight')
                : $(this).width();
    }

    $.fn.outerHeight = function(needMargin) {
        return needMargin === true ? 
                $(this).height() + curCSS(this[0], 'marginTop') + curCSS(this[0], 'marginBottom')
                : $(this).height();
    }
})(af);

/*
    add/augment some static methods for app framework.
 */
(function($) {
     ...
     */
    $.trim = function(str) {
        return core_trim ? str.trim() : str.replace(TRIM_RE, '');
    }
     ...
     */
    $.isArray = function(obj) {
        return obj != null && (isArray ? isArray(obj) : core_toString.call(obj) === '[object Array]');
    }
    ...
    /**
     * 扩展对象属性，详见：http://api.jquery.com/jQuery.extend/
     * @return {[type]} [description]
     */
    $.extend = function() {
        ...
        css3Animate.prototype = {
            animate: function(options) {
                ...
                if (classMode) {
                    ...
                    var trans = "translate" + translateOpen + (options.x) + "," + (options.y) + translateClose + " scale(" + parseFloat(options.scaleX) + ',' + parseFloat(options.scaleY) + ") rotateX(" + options.rotateX + ") rotateZ(" + options.rotateZ + ")";
                    if (!$.os.opera)
